...
function plugin(opts){
  ...
  return function(files, metalsmith, done){
    ...
    /**
     * Initialize empty collections (this deletes previous collection data if the plugin has already been called)
     */
    ...
    keys.forEach(function(key){
      ...
      col.forEach(function(file, i){
        var navPropSuffix = settings.suffixNavigationProps === true
          ? key[0].toUpperCase() + key.slice(1)
          : '';
        if (0 != i) {
          file['previous' + navPropSuffix] = col[i-1];
        }
        if (last != i) {
          file['next' + navPropSuffix] = col[i+1];
        }
      });
    });
